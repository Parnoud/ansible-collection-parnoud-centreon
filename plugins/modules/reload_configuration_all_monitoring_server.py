...
EXAMPLES = r'''
---
- name: Reload the configuration files for all monitoring servers
  parnoud.centreon.reload_configuration_all_monitoring_server:
...
from ansible_collections.parnoud.centreon.plugins.module_utils.monitoring_server import reload_configuration_all_monitoring_server


def _reload_configuration_all_monitoring_server(module):
    ...
    if reload_configuration_all_monitoring_server(api):
        return True, 'OK'
    else:
        return False, 'NOK'
...
def main():
    ...
    status, result = _reload_configuration_all_monitoring_server(module)
    if status:
