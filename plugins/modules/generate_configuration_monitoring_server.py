...
DOCUMENTATION = r'''
---
module: generate_configuration_monitoring_server
short_description: Generate and move the configuration files of the monitoring server
description:
    - Generate and move the configuration files of the monitoring server by id
...
- name: Generate and move the configuration files of the monitoring server
  parnoud.centreon.generate_configuration_monitoring_server:
...
from ansible_collections.parnoud.centreon.plugins.module_utils.monitoring_server import generate_configuration_monitoring_server


def generate_configuration_monitoring_server_by_id(module):
    ...
    if generate_configuration_monitoring_server(api, monitoring_server_id=module.params.get('monitoring_server_id')):
        return True, 'OK'
    else:
        return False, 'NOK'
...
def main():
    ...
    status, result = generate_configuration_monitoring_server_by_id(module)
    if status:
